...
// btnSaveReport______________________________________________________________
        ...
        btnSaveReport.addActionListener(e -> {
            switch (tp.getSelectedIndex()) {
                case 0 -> new MyChooser(parentFrame).writeTextFile(parentFrame.getPathWorkDir(),
                        "kat",
                        parentFrame.getTitles().get("SVRsaveTitle0"),
                        parentFrame.getPolygonProject().getReportNXYZ());
                case 1 -> new MyChooser(parentFrame).writeTextFile(parentFrame.getPathWorkDir(),
                        "txt",
                        parentFrame.getTitles().get("SVRsaveTitle1"),
                        parentFrame.getPolygonProject().getReportXY());
                case 2 -> new MyChooser(parentFrame).writeTextFile(parentFrame.getPathWorkDir(),
                        "txt",
                        parentFrame.getTitles().get("SVRsaveTitle1"),
                        parentFrame.getPolygonProject().getReportZ());
...
// spReportXY_______________________________________________________________________
        ...
        LinkedList<String> llReportXY = parentFrame.getPolygonProject().getReportXY();
...
// spReportZ_________________________________________________________________________
        ...
        LinkedList<String> llReportZ = parentFrame.getPolygonProject().getReportZ();
...
// tblNXYZ_______________________________________________________________________
            ...
            for (int i = 0; i < parentFrame.getPolygonProject().getSizeTheoStations(); i++) {
                catalogNXYZ.add(new CatalogPoint(parentFrame.getPolygonProject().getTheoStation(i).getName(),
                        parentFrame.getPolygonProject().getTheoStation(i).getX(),
                        parentFrame.getPolygonProject().getTheoStation(i).getY(),
                        parentFrame.getPolygonProject().getTheoStation(i).getZ()));
            }
